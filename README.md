...
[![Version](https://img.shields.io/badge/version-v0.5.1--beta.2-orange.svg)](https://github.com/hrdsht/daw_buddy/releases)
...
**From then on, just double-click the launcher** in the `scripts/` folder
instead of using a terminal:

- **Windows:** `scripts/DAW Buddy.bat`
- **macOS:** `scripts/DAW Buddy.command` — run `chmod +x "scripts/DAW Buddy.command"` once so macOS will let it launch
- **Linux:** `scripts/DAW Buddy.sh` — run `chmod +x "scripts/DAW Buddy.sh"` once to make it executable
...
├── docs/                 build notes, technical proposals, user guide
...
## Part 6 — Windows, macOS, and Linux

The app runs consistently across all three platforms and adapts where they differ:

- **Paths** are built with Node's `path` module, never by gluing strings, so
  they're correct on all operating systems.
- **Case.** Windows and macOS treat `C:\Projects` and `c:\projects` as the same
  folder; native Linux filesystems (ext4/btrfs) do not. Path comparisons lowercase
  themselves only where that's true.
- **Wording.** The reveal button says "Show in Finder" on macOS, "Show in File
  Explorer" on Windows, and "Show in File Manager" on Linux.
- **File watching.** Windows uses ReadDirectoryChangesW, macOS uses FSEvents, Linux
  uses inotify; chokidar handles the difference. Network shares send neither —
  that's what the polling toggle in Settings is for.
- **Permissions.** macOS asks for permission on protected folders (Documents, Desktop);
  Linux respects user/group read permissions for mounted drives.
