 ...
 *            includeExtension, template, parentFolder, bpm, key }
 */
...
function transform(stem, options, index) {
  switch (options.operation) {
    ...
    case 'applyTemplate': {
      let out = options.template || '{name}';
      
      // {name} inherently preserves the exported timeline item name
      out = out.replace(/{name}/g, stem);
      out = out.replace(/{parent}/g, options.parentFolder || '');
      out = out.replace(/{project}/g, options.projectName || '');
      out = out.replace(/{bpm}/g, options.bpm || '');
      out = out.replace(/{key}/g, options.key || '');
      
      // {n}, and padded variants. Accepts {n:02} and {n:2} alike — the
      // leading zero is how people write it, but demanding it silently
      // leaves {n:2} unreplaced in the filename.
      out = out.replace(/{n(?::0?(\d+))?}/g, (match, pad) => {
        const num = String((Number(options.startAt) || 1) + index);
        return pad ? num.padStart(Number(pad), '0') : num;
      });
      
      // Generates a clean YYYY-MM-DD date stamp
...
async function apply(planned, undoLogPath) {
  ...
  for (const row of doing) {
    if (!isSafeFileName(row.to)) {
      failed.push({ ...row, message: 'The new name must be a plain filename' });
      continue;
    }
...
async function undo(undoLogPath) {
  ...
  for (const move of log.moves.slice().reverse()) {
    try {
      if (await occupiedByOtherFile(move.from, move.to)) {
        throw new Error('The original path is occupied by another file');
      }
  ...
  await fs.writeFile(
    undoLogPath,
    JSON.stringify({ moves: remaining.reverse() }, null, 2),
    'utf8'
  );
...
async function occupiedByOtherFile(target, current) {
  try {
    const [targetStat, currentStat] = await Promise.all([
      fs.stat(target),
      fs.stat(current)
    ]);
    return targetStat.dev !== currentStat.dev || targetStat.ino !== currentStat.ino;
  } catch (err) {
